...
from Qt.QtWidgets import QWidget, QToolButton, QMenu, QAction
...
class JointWidget(base.CommandRigToolBoxWidget, object):
    ...
    def _setup_display_tools(self):
        ...
        local_axis_on_icon = resources.icon('local_rotation_on')
        local_axis_off_icon = resources.icon('local_rotation_off')
        local_axis_on_all_icon = resources.icon('local_rotation_on_all')
        local_axis_off_all_icon = resources.icon('local_rotation_off_all')
        local_axis_on_selected_icon = resources.icon('local_rotation_on_selection')
        local_axis_off_selected_icon = resources.icon('local_rotation_off_selection')
        ...
        joint_icon = resources.icon('joint', extension='png')
    ...
    def _setup_orient_tools(self):
        ...
        orient_icon = resources.icon('orient_joints')
        orient_all_icon = resources.icon('orient_all_joints')
        orient_selected_icon = resources.icon('orient_selected_joints')
        ...
        add_icon = resources.icon('plus', extension='png')
        ...
        clean_icon = resources.icon('clean', extension='png')
        ...
        orient_tool_icon = resources.icon('orient_tool', extension='png')
...
class JointWidgetModel(QObject, object):
    ...
    def __init__(self):
        ...
        try:
            self._joints_display_size = dcc.get_joint_display_size()
        except Exception:
...
class JointWidgetController(object):
    ...
    def set_joint_display_size(self):
        ...
        return dcc.set_joint_display_size(joints_display_size)

    def select_hierarchy(self):
        return dcc.select_hierarchy()
    ...
    def mirror_selected_joints(self):
        ...
        selected = dcc.selected_nodes_of_type('transform', full_path=False)
        if not selected:
            LOGGER.warning('Please select joints to mirror!')
            return

        return dcc.mirror_transform(transforms=selected)
    ...
    def mirror_hierarchy_joints(self):
        ...
        all_xforms = list()
        selected = dcc.selected_nodes_of_type(node_type='transform') or list()
        for xform in selected:
            all_xforms.append(xform)
            children = dcc.list_children(full_path=False, children_type='transform')
            found = list()
            if children:
        ...
        return dcc.mirror_transform(transforms=all_xforms)
    ...
    def mirror_create_joints(self):
        ...
        selected = dcc.selected_nodes_of_type(node_type='transform') or list()
        created = dcc.mirror_transform(transforms=selected, create_if_missing=True)
    ...
    def mirror_create_all_joints(self):
        ...
        created = dcc.mirror_transform(create_if_missing=True)
        if created:
            LOGGER.info('Created transforms on the right side that were on the left')
        else:
            LOGGER.warning(
    ...
    def mirror_joints(self):
        ...
        selected = dcc.selected_nodes_of_type(node_type='joint') or list()
    ...
    def orient_all_joints(self, force_orient_attributes=True):
        ...
        return dcc.orient_joints(force_orient_attributes=force_orient_attributes)
    ...
    def orient_selected_joints(self, force_orient_attributes=True):
        ...
        selected = dcc.selected_nodes_of_type('joint', full_path=False)
        if not selected:
            LOGGER.warning('Please select joints to orient')
            return

        oriented = dcc.orient_joints(joints_to_orient=selected, force_orient_attributes=force_orient_attributes)

        dcc.select_node(selected)
    ...
    def zero_selected_joint_orient(self):
        ...
        return dcc.zero_orient_joint(selected)
    ...
    def zero_all_joint_orient(self):
        ...
        return dcc.zero_orient_joint(all_joints)
    ...
    def toggle_all_local_rotation_axis(self, value=None):
        ...
        return dcc.set_joint_local_rotation_axis_visibility(value)
    ...
    def toggle_selected_local_rotation_axis(self, value=None):
        ...
        return dcc.set_joint_local_rotation_axis_visibility(value, joints_to_apply=selected)
    ...
    def toggle_xray(self):
        return dcc.toggle_xray()

    def toggle_selected_xray(self):
        return dcc.toggle_xray_on_selection()

    def toggle_xray_joints(self):
        return dcc.toggle_xray_joints()
